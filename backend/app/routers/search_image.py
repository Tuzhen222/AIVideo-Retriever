...
from app.schemas.search_image import ImageSearchRequest, ImageSearchResponse
...
async def get_image_embedding(image_path: str) -> List[float]:
    ...
    Args:
        image_path: Path to image file (e.g., "/keyframe/L01_V001/123.webp")
    Returns:
        embedding: List of floats (CLIP image embedding)
    """
    # Convert URL path to file system path
    # Frontend sends: "/keyframe/L01_V001/123.webp"
    # Need to convert to: "backend/app/data/keyframe/L01_V001/123.webp"
    
    if image_path.startswith("/keyframe/"):
        # Remove "/keyframe/" prefix and add proper data directory path
        relative_path = image_path.replace("/keyframe/", "")
    ...
    model_server_url = "https://fetching-federico-ruffianly.ngrok-free.dev"
...
async def search_by_image_upload(
    ...
    try:
        ...
        logger.info(f"[IMAGE_SEARCH] Calling model server: {endpoint}")
        ...
        logger.error(f"[IMAGE_SEARCH] Model server HTTP error: {type(e).__name__} - {str(e)}")
        if hasattr(e, 'response') and e.response is not None:
            logger.error(f"[IMAGE_SEARCH] Response status: {e.response.status_code}")
            logger.error(f"[IMAGE_SEARCH] Response body: {e.response.text[:500]}")
        raise HTTPException(status_code=500, detail=f"Failed to get image embedding: {str(e)}")
    except Exception as e:
        logger.error(f"[IMAGE_SEARCH] Error during search: {e}")
        raise HTTPException(status_code=500, detail=f"Image search failed: {str(e)}")

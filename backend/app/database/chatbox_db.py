    ...
    try:
        with get_connection() as conn:
            ...
            if row:
                ...
                raise ValueError(f"Failed to retrieve created submission with id {submission_id}")
...
def get_submissions(
    ...
    try:
        with get_connection() as conn:
            ...
            # Get total count
            cursor.execute(f"SELECT COUNT(*) FROM query_submissions WHERE {where_clause}", params)
...
def delete_submission(submission_id: int) -> bool:
    ...
    try:
        with get_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("DELETE FROM query_submissions WHERE id = ?", (submission_id,))

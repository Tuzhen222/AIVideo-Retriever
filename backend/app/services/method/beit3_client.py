...
class BEiT3Client:
    ...
    def extract_text_embedding(
        ...
        if self._batch_available is None:
            try:
                url = f"{self.base_url}/embedding/beit3/text/batch"
                payload = {"texts": texts}
                ...
                if response.status_code == 200:
                    ...
                    logger.info(f"[BEiT3] Batch endpoint not available, using individual calls")
            except Exception as e:
                self._batch_available = False
                logger.info(f"[BEiT3] Batch endpoint not available: {e}")
        ...
        # Use batch endpoint if available
            ...
            try:
                ...
                logger.error(f"[BEiT3] Batch request failed: {e}, falling back")
